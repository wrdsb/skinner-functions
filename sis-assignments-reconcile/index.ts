...
const sisAssignmentsReconcile: AzureFunction = async function (context: Context, triggerMessage: string): Promise<void> {
    ...
    async function createEvent(totalDifferences)
    {
        ...
        let event = {
            ...
            eventType: 'Skinner.Assignments.Differences.Reconcile',
            ...
            data: {
                ...
                data: {
                    ...
                    blob: {
                        path: `trillium/sis-assignments-reconcile.json`,
    ...
    async function consolidateCosmosItems(items: any[], consolidatedObject)
    {
        items.forEach(function(item) {
            if (!item.deleted) {
                ...
                consolidatedObject[item.id] = sisAssignment;
            }
        });
...
export default sisAssignmentsReconcile;
