...
const materializeSchoolClasses: AzureFunction = async function (context: Context, triggerMessage: string): Promise<void> {
    const execution_timestamp = (new Date()).toJSON();  // format: 2012-04-23T18:25:43.511Z

    const storageAccount = process.env['storageAccount'];
    const storageKey = process.env['storageKey'];
    const blobContainer = 'school-classes';
    ...
    {
        const blobName = schoolCode.toLowerCase() + '-school-classes.json';
    ...
    async function createEvent(logObject)
    {
        ...
        let event = {
            ...
            eventType: 'Skinner.School.Classes.Materialize',
...
export default materializeSchoolClasses;
