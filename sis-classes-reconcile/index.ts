...
const sisClassesReconcile: AzureFunction = async function (context: Context, triggerMessage: string): Promise<void> {
    ...
    async function createEvent(totalDifferences)
    {
        ...
        let event = {
            ...
            eventType: 'Skinner.Class.Differences.Reconcile',
    ...
    async function consolidateCosmosItems(items: any[], consolidatedObject)
    {
        items.forEach(function(item) {
            if (!item.deleted) {
                ...
                consolidatedObject[item.id] = sisClass;
            }
        });
...
export default sisClassesReconcile;
