...
const materializeSchoolStudents: AzureFunction = async function (context: Context, triggerMessage: string): Promise<void> {
    const execution_timestamp = (new Date()).toJSON();  // format: 2012-04-23T18:25:43.511Z

    const storageAccount = process.env['storageAccount'];
    const storageKey = process.env['storageKey'];
    const blobContainer = 'school-students';
    ...
    {
        const blobName = schoolCode.toLowerCase() + '-school-students.json';
    ...
    async function createEvent(logObject)
    {
        ...
        let event = {
            ...
            eventType: 'Skinner.School.Students.Materialize',
...
export default materializeSchoolStudents;
